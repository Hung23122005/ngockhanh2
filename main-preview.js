(() => {
  ...
  function initMatrix() {
    ...
    matrix.columns = Array.from({ length: count }, (_, i) => ({
  ...
  function drawMatrix(now, dt) {
    ...
    matrix.columns.forEach((c, ci) => {
      ...
      for (let j = 0; j < c.len; j++) {
        ...
        const idx =
          Math.abs(Math.floor(now / 115) + ci * 7 + j * 3) % glyphs.length;

        mctx.fillText(glyphs[idx], c.x, yy);
      }
    });
  }
      ...
      const next = this.items.map((p) => {
        ...
        if (mode === "horizontal") {
          ...
          /*
                ...
                Instead of particles flying
                directly off-screen, the portrait
                breaks into a thick floating cloud.

                This is the effect used between
                photos.
              */
          ...
          const a =
            Math.atan2(cur.y - H / 2, cur.x - W / 2) +
            (Math.random() - 0.5) * 1.4;
    ...
    CFG.photos.forEach((src, index) => {
      ...
      button.addEventListener("click", () => {
        if (performance.now() >= suppressLanternClickUntil) {
          if (pendingMemoryOpen) clearTimeout(pendingMemoryOpen);
          ...
          pendingMemoryOpen = setTimeout(() => {
            ...
            if (lanternStage.classList.contains("active")) openMemory(index);
          }, 340);
        }
      });
  ...
  lanternStage.addEventListener("pointerdown", (event) => {
    if (memoryViewer.classList.contains("open")) return;
  ...
  lanternStage.addEventListener("pointermove", (event) => {
    ...
    if (dragDistance > 4) {
      ...
      sceneYaw = window.Scene3D.yawFromDrag(sceneYaw, deltaX, lanternStage.clientWidth);
  ...
  function endSceneDrag(event) {
    if (event.pointerId !== dragPointer) return;
    if (dragDistance > 4) suppressLanternClickUntil = performance.now() + 300;
    dragPointer = null;
  ...
  addEventListener("keydown", (e) => {
    ...
    if (e.key === "ArrowLeft") stepMemory(-1);
    if (e.key === "ArrowRight") stepMemory(1);
  });
  ...
  function updateLanterns(time) {
    ...
    lanternButtons.forEach((button, index) => {
      ...
      const edgeFade = Math.min(1, Math.max(0, (screenY + 90) / 90), Math.max(0, (H + 90 - screenY) / 90));
      button.style.left = `${rect.left + point.x * rect.width / 900}px`;
      button.style.top = `${screenY}px`;
      button.style.setProperty("--scale", Math.max(0.52, Math.min(1.7, point.scale * (0.84 + (index % 4) * 0.07))).toFixed(3));
  ...
  /*
    ...
    0.00 -> particles form image
    1.08 -> particle image is fully formed
    2.20 -> particles begin to loosen
    2.28 -> particles explode into cloud
    3.20 -> next photo
  */
  ...
  function updatePhotoTimeline(t, now) {
    ...
    // The old automatic photo carousel is intentionally disabled here.
    // From this point onward the animation becomes an interactive blossom island.
    photoStage.classList.remove("active");
    lanternStage.classList.add("active");
    lanternStage.setAttribute("aria-hidden", "false");
    heroFrame.style.opacity = "0";

    // Let the previous particle blast fade away behind the tree.
  ...
  function frame(now) {
    ...
    if (paused) {
      ...
      morph.draw(
        getPausedMorphTime(now, paused, pauseStarted),
        photoFxAlpha,
      );
